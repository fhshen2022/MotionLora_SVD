...
class MultiMotionDataset(Dataset):
    def __init__(self, base_folder: str, labels: dict, motion_dir: str, num_samples=100000, width=1024, height=576, sample_frames=25):
        """
        Args:
            base_folder (str): Path to the base folder containing subfolders for each label.
            labels (dict): Dictionary mapping folder names to labels.
            num_samples (int): Number of samples in the dataset.
            width (int): Width of the frames.
            height (int): Height of the frames.
            sample_frames (int): Number of frames to sample from each video.
        """
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            dict: A dictionary containing 'pixel_values' tensor and its corresponding label.
        """
        # Randomly select a folder (representing a video) from the base folder
        selected_folders = chosen_folder = [entry for entry in self.folders if os.path.isdir(os.path.join(self.base_folder, entry))]
        chosen_folder = random.choice(selected_folders)
        motion_folder_path = os.path.join(self.base_folder, chosen_folder, self.motion_dir)
        chosen_video_path = random.choice(os.listdir(motion_folder_path))
        folder_path = os.path.join(motion_folder_path,chosen_video_path)
        #print(folder_path)
        frames = os.listdir(folder_path)

        # Assign label based on the folder name
        #print(chosen_folder)
        label = self.labels.get(chosen_folder, 2)  # Default to -1 if folder name is not in labels
        ...
        ''''''
        new_frames = []
        for vid_name in frames:
            index = vid_name.split('.')[0]
            #if int(index)%3==1 or int(index)%3==2:
            if int(index)%label==0:
                #print(index)
                new_frames.append(vid_name)
        frames = new_frames
        #print(folder_path,frames)
        ...
        return {'pixel_values': pixel_values, 'motion_scale': label}
...
def main():
    ...
    #######added by sfh#########
    lora_layer=[]
    for name, module in unet.named_parameters():
        if 'temporal_transormer_block' in name:
    ...
    train_dataset = MultiMotionDataset(args.base_folder, width=args.width, height=args.height, sample_frames=args.num_frames)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                motion_scale = batch['motion_scale'][0].item()
                #print(motion_scale)
                ...
                first_frame_latents = conditional_latents.unsqueeze(1)
                #
                ...
                consistency_losses = torch.stack([
                    torch.mean((denoised_latents[:, i:i+1, :, :, :] - first_frame_latents) ** 2)
                    for i in range(denoised_latents.shape[1])
                ])


                # 根据时间步和帧位置调整一致性损失的权重
                frame_positions = torch.arange(denoised_latents.shape[1], device=latents.device)
                frame_position_adjustment = 1 / (1 + frame_positions)  # 帧越大约束越弱

                # 时间步调整
                time_adjustment = 1 / (1 + timesteps)  # 时间步越大约束越弱

                # 结合时间步和帧位置的调整因子
                #print(consistency_losses)
                adjusted_consistency_losses = consistency_losses * time_adjustment[:, None] * frame_position_adjustment
                #print(adjusted_consistency_losses)

                loss = loss + 0.5*torch.mean(adjusted_consistency_losses)
                #print(loss)
                ################### end #################
                '''
                '''
                ##############adjacent consistency loss, added by sfh######################
                # 计算每一帧与前一帧之间的一致性损失（从第二帧开始）
                consistency_loss_adjacent = torch.mean(
                    torch.stack([torch.mean((denoised_latents[:, i:i+1, :, :, :] - denoised_latents[:, i-1:i, :, :, :]) ** 2)
                                for i in range(1, denoised_latents.shape[1])])
                )
                #print(loss,0.2*consistency_loss_adjacent)
                loss = loss + 0.3*consistency_loss_adjacent
                ###########################end#######################
                '''
